...
$.SystemApp.initContentArea = function() {
    ...
    $("body").on("DOMNodeInserted", ".con-main",
	    function(e) {
...
$.SystemApp.commonOper = {
		showEdit:function(isEdit,urllink,formobject,title){
			...
			var dialogObj =$.SystemApp.openDialog($.SystemApp.contextPath
					+ urllink, {
				data : _params,
				height : 500,
				width : 700,
		...
		save:function(urllink,formobject){
			...
			$.post($.SystemApp.contextPath + urllink,
							data, function(data) {
								if (data.successRows > 0) {
									alert("数据保存成功！");
...
$.SystemApp.collClueJscript = {
		...
		showEdit : function(isEdit) {
			var _this = this;
			_this.dialogObj = $.SystemApp.commonOper.showEdit(isEdit,"/collclue/showEdit","tableHover_collClue","中文标题编辑");
...
$.SystemApp.commAttachmentsJscript = {
		dialogObj : null,
		init_UI : function() {
				$.SystemApp.divLoad("#systemData", "/commattachments/list", '',
					function() {
		...
		showEdit : function(isEdit) {
			var _this = this;
			_this.dialogObj = $.SystemApp.commonOper.showEdit(isEdit,"/commattachments/showEdit","tableHover_commAttachments","中文标题编辑");

		},
		del : function() {
			$.SystemApp.commonOper.del("/commattachments/delete","tableHover_commAttachments");
			
		},
		save : function(formObject) {
			$.SystemApp.commonOper.save("/commattachments/update",formObject);
			
		},
		find : function(formobject) {
			var data = $("#"+formobject).serialize();

			$.SystemApp.divLoad("#systemData", "/commattachments/list", data,
					function() {
