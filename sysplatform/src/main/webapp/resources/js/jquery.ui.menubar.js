			...
			this._on( anItem, {
				click: function(event) {
					if ( this.active ) {
						...
						if (this.activeItem){
							this.activeItem.removeClass("ui-button-text-icon-primary");
							this.activeItem.children(".ui-button-icon-primary").remove();
						}
						if (this.options.selecticons){
							$( anItem ).addClass("ui-button-text-icon-primary");
							$( anItem ).prepend('<span class="ui-button-icon-primary ui-icon '+this.options.selecticons.select+'"></span>');
							this.activeItem = anItem;
						}
						this._trigger( "select", event, $(anItem));
					}
				},
				mouseenter: function() {
					if ( this.open ) {
