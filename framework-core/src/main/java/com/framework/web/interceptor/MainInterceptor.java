...
public class MainInterceptor extends HandlerInterceptorAdapter {
	...
	public void postHandle(HttpServletRequest _request,
			...
			ModelAndView _modelAndView) throws Exception {
...
//			System.out.println(String.format("%s consume %d millis", _request.getRequestURI(), consumeTime));		
//		}	
		...
		if (classes.length>0){
			for(Class<?> classz:classes){
				if (classz.equals(BindingResult.class)){
					...
					MethodParameter methodParam = handlerMethod.getMethodParameters()[1];
					methodParam.getParameterName();
					Exception thrownException=null;
					GenericTypeResolver.resolveParameterType(methodParam, handlerMethod.getClass());
